...
sub _connect {
  ...
  $id = $self->_loop->client(
    ...
    tls_key       => $self->key,
    sub {
<div class='code-clone-start 5ce7192ac44d3c98462d4abdb52920fa'></div>      my ($loop, $err, $stream) = @_;

      # Events
      return $self->_error($id, $err) if $err;
      $self->_events($stream, $id);
<div class='code-clone-end 5ce7192ac44d3c98462d4abdb52920fa'></div>      $self->_connected($id);
    }
  );
  $self->{connections}->{$id} = {cb => $cb, tx => $tx};
...
<div class='code-clone-start 5ce7192ac44d3c98462d4abdb52920fa'></div>            my ($loop, $err, $stream) = @_;
            ...
            return $self->_error($id, $err) if $err;
            $self->_events($stream, $id);
<div class='code-clone-end 5ce7192ac44d3c98462d4abdb52920fa'></div>
...
=head1 SYNOPSIS
  ...
  say $ua->get('www.â.net?hello=there')->res->body;
