...
sub write {
<div class='code-clone-start f19b481ec024c445bba6e52c698213dc'></div>  my ($self, $chunk, $cb) = @_;

  if (ref $chunk && ref $chunk eq 'CODE') {
    $cb    = $chunk;
    $chunk = undef;
  }
<div class='code-clone-end f19b481ec024c445bba6e52c698213dc'></div>  $self->res->write($chunk, sub { shift and $self->$cb(@_) if $cb });
  $self->rendered;
...
<div class='code-clone-end f19b481ec024c445bba6e52c698213dc'></div>  $self->res->write_chunk($chunk, sub { shift and $self->$cb(@_) if $cb });
  $self->rendered;
